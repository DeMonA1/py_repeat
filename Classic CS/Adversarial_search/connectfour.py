...
def generate_segments(num_columns: int, num_rows: int, segment_length: int) -> \
    ...
    # generates diagonal segments from down left to top right corner
    for c in range(num_columns - segment_length + 1):
        for r in range(num_rows - segment_length + 1):
            segment = []
            for t in range(segment_length):
    ...
    # generates diagonal segments from top left to down right corner
    for c in range(num_columns - segment_length + 1):
...
class C4Board(Board):
    ...
    # Return amount of red and black chips in the segment
