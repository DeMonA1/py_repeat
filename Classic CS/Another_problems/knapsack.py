...
def knapsack(items: List[Item], max_capacity: int) -> List[Item]:
    # building the table of dynamic programming
    table: List[List[float]] = [[0.0 for _ in range(max_capacity + 1)] 
                                for _ in range(len(items) + 1)]
    for i, item in enumerate(items):
        for capacity in range(1, max_capacity + 1):
            previous_items_value: float = table[i][capacity]
            if capacity >= item.weight:     # item fits in a backpack
                value_freeing_weight_for_item: float = table[i][capacity - item.weight]
                # only if this item valuable than the previous
                table[i + 1][capacity] = max(value_freeing_weight_for_item + item.value,
                                             previous_items_value)
            else:       # there is no place for this item
                table[i + 1][capacity] = previous_items_value
    # find the solution in a table
    solution: List[Item] = []
    capacity = max_capacity
    for i in range(len(items), 0, -1):          #  going to the opposite direction
        # have this item chosen?               
        if table[i - 1][capacity] != table[i][capacity]:
            solution.append(items[i - 1])
            # if item have chosen, then subtract its weight
