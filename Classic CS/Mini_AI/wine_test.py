...
if __name__ == "__main__":
    ...
    with open('wine.csv', mode='r') as wine_file:
        ...
        shuffle(wines)          # receive our data strings in random order
        for wine in wines:
    ...
    normalize_by_future_scaling(wine_parameters)
    
    wine_network: Network = Network([13, 7, 3], 0.9)
    
    # learning on the first 150 samples of wine, repeat 10 times
    ...
    # test on the last 28 wine samples in the data set
    wine_testers: List[List[float]] = wine_parameters[150:178]
    wine_testers_correcst: List[int] = wine_species[150:178]
    wine_results = wine_network.validate(wine_testers, wine_testers_correcst, wine_interpret_output)
