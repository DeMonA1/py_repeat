...
# compresse data of 165 bytes
PEOPLE: List[str] = ['Michael', 'Sarah', 'Joshua', 'Narine', 'David',
                     'Sajid', 'Melanie', 'Daniel', 'Wei', 'Dean',
                     'Brian', 'Murat', 'Lisa']
...
class ListCompression(Chromosome):
    ...
    def mutate(self) -> None:       # swap 2 elements
        idx1, idx2 = sample(range(len(self.lst)), k=2)
...
if __name__ == "__main__":
    ...
    ga: GeneticAlgorithm[ListCompression] = GeneticAlgorithm(initial_population=
            initial_population, threshold=1.0, max_generations=1000, mutation_chance=0.2,
